...
import numpy as np
import yaml
from sklearn.cluster import OPTICS  # , MeanShift, estimate_bandwidth, DBSCAN
...
class Cluster:
    ...
    This is similar to a node in a tree data structure.
    """
    ...
    def __init__(
        ...
        h5_data: Optional[h5py.Dataset] = None,
    ):
        self.id = id
        self.time = time
        ...
        self.descendant = None  # Descendent (next in time) cluster
        self.projenitors = []  # List of cluster objects directly preceding this one in time
        self.mass_hist = 0
    ...
    def get_root(self):
        """Get the root of tree this cluster belongs to
        ...
        elif self.descendant is None:  # No descendents means this is the root
            self.root_descendant = self
            return self
        else:  # Root descendent has not been set yet, use descendent to set root descendent
            self.root_descendant = self.descendant.get_root()
            return self.root_descendant

    def compare(self, cluster_b: "Cluster") -> int:
        return len(set(self.part_ids).intersection(set(cluster_b.part_ids)))

    def get_largest_branch(self) -> List["Cluster"]:
    ...
    def get_all_progenitors(self) -> List["Cluster"]:
        """Recursively collect and return a list of all 'progenitor' clusters
...
class ClusterTree(object):
    ...
    def get_branch_roots(self) -> List["Cluster"]:
        """Return list of clusters that are the roots of branches
    ...
    def prune_branches(self, min_n_progs=3):
        ...
        for root in del_roots:
            ...
            if root.descendant is not None:  # If root was merged
...
def find_descendants(
    clusters: List["Cluster"], thresh: float = 0.6, nskip: int = 1
) -> List["Cluster"]:
    """
    ...
    clusters : List[&quot;Cluster&quot;]
        List of clusters at each time point
...
def identify_spatial_clusters(
    com_arr, eps=0.05, min_samples=12, thresh=20, verbose=True, **kwargs
):
...
def collect_cluster_data(
    run_path, ss_ind=1, end_ind=None, start_bead=0, end_bead=None, **kwargs
):
...
def make_cluster_trees(
    clusters, thresh=0.1, nskip=20, tree_min_size=20, min_progs=3, **kwargs
):
...
def create_cluster_hdf5(
    ...
    **kwargs,
    ...
    clust_path = anal_file_path.parent / f"cluster_{anal_file_path.parent.stem}.h5"
...
def create_cluster_yaml(
    anal_file_path, ss_ind=1, end_ind=-1, start_bead=0, end_bead=None
):
    with h5py.File(anal_file_path, "r+") as h5_data:
        time_arr = h5_data["time"][ss_ind:end_ind]
        print(time_arr.shape)
