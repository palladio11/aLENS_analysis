...
from .chrom_condensate_analysis import (
...
def make_all_condensate_graphs(h5_data, opts, overwrite=False):
    ...
    plot_condensate_kymo(
        axarr1[1], pos_cond_edge_coords, ylims=(bin_centers[0], bin_centers[-1])
    )
    ...
    plot_condensate_tracks(
        ...
        time_arr,
        pos_cond_lst,
        ylims=(bin_centers[0], bin_centers[-1]),
        legend_flag=False,
        ylabel=None,
    )
    ...
    if contact_mat is not None:
        fig3, axarr3 = make_summed_contact_kymo_graph(
            ...
            vmin=-25,
            vmax=7,
            avg_contact_mat=log_avg_contact_mat,
            avg_vmin=-7,
        )
    ...
    if "max_contact_cond_size" not in analysis_grp:
        max_contact_cond_size, total_contact_cond_beads = get_max_and_total_cond_size(
            time_arr, contact_cond_edges, contact_cond_num, analysis=analysis_grp
        )
    else:
    ...
    plot_condensate_avg_contact_vs_time(
        axarr5[1, 0], time_arr, contact_kymo, cond_lst, same_start_flag=False
    )
...
def make_total_distr_plots(
    com_arr, log_contact_avg=None, hist_max=1.0, rel_ind=0, vmin=-50
):
...
def make_min_distr_plots(
    com_arr, log_contact_avg=None, hist_max=1.0, rel_ind=0, vmin=-30
):
...
def make_segment_distr_graphs(com_arr, sep_ids, rel_ids, e0_ind, e1_ind, hist_max=1.0):
    ...
    zero_pos = 0.5 * (com_arr[rel_ids[0]] + com_arr[rel_ids[1]])
    ...
    for i in range(n_rows):
        ...
        rad_hist, rad_edges = rad_distr_hists(seg_com_arr, zero_pos, hist_max=hist_max)
        rad_mean = np.average(rad_edges[:-1] + np.diff(rad_edges), weights=rad_hist)
        axarr[i * 3 + 2].bar(
            rad_edges[:-1], rad_hist, width=np.diff(rad_edges), align="edge"
        )
        axarr[i * 3 + 2].set_ylabel(r"Probability density ($\mu$m$^{-1}$)")
        axarr[i * 3 + 2].set_xlabel(r"$|{\bf r} - {\bf r}_0|$ ($\mu$m)")
        axarr[i * 3 + 2].axvline(
            rad_mean, color="r", label=f"mean = {rad_mean:.4} $\mu$m"
        )
...
def make_summed_contact_kymo_graph(
    ...
    X, Y = np.meshgrid(x, x)
    c1 = axarr[0].pcolorfast(
        X, Y, np.log(contact_mat[:, :, -1]) / np.log(10), vmin=vmin
    )
    ...
    plot_contact_kymo(
        fig, axarr[1], time_arr, contact_kymo, contact_type=contact_type, vmax=vmax
    )
...
def plot_link_energy_vs_time(ax, h5_data, ss_frac=0.75):
...
def plot_condensate_kymo(
    ax, edge_coords, xlims=None, ylims=None, axis_label_flag=True, ylabel=""
):
    ...
    if len(edge_coords) > 0:
        ...
        ax.scatter(
            edge_coords[:, 0],
            0.5 * (edge_coords[:, 2] + edge_coords[:, 1]),
            label="center",
        )
        ax.vlines(
            edge_coords[:, 0],
...
def plot_condensate_characterize(
    ...
    ax0.plot(time_arr, max_cond_size, color="b", linestyle="--", label="Largest")
...
def plot_condensate_tracks(
    ax, time_arr, cond_lst, ylims=None, legend_flag=True, ylabel="Bead index"
):
    ...
    for cond in cond_lst:
        ax.plot(
            cond.time_arr,
            0.5 * np.asarray(cond.edge_coord_arr).sum(axis=1),
            label=f"id = {cond.id}",
        )
...
def plot_condensate_size_vs_time(
    ...
    for cond in cond_lst:
        ...
        try:
            size_arr = (cond.edge_coord_arr[:, 1] - cond.edge_coord_arr[:, 0]) + 1
            start_cond_ind = np.where(time_arr == cond.time_arr[0])[0][0]
            end_cond_ind = np.where(time_arr == cond.time_arr[-1])[0][0]
            move_time = 0.0 if not same_start_flag else time_arr[start_cond_ind]
            _ = ax.plot(
...
def plot_condensate_avg_contact_vs_time(
    ax, time_arr, contact_kymo, cond_lst, same_start_flag=False, **kwargs
):
    for cond in cond_lst:
        cond.edge_coord_arr = np.asarray(cond.edge_coord_arr)
        try:
...
def graph_clust_snapshot(
    fig, axarr, com_arr, clust, cluster_centers, cluster_member_inds
):
    ...
    _ = axarr[0].scatter(com_arr[:, 0], com_arr[:, 1], s=0.5)
    _ = axarr[1].scatter(com_arr[:, 0], com_arr[:, 2], s=0.5)
    ...
    for k, my_members, col in zip(range(n_clusters), cluster_member_inds, colors):
        ...
        _ = axarr[2].scatter(
            com_arr[my_members, 0], com_arr[my_members, 1], color=col, s=0.5
        )
        _ = axarr[3].scatter(
            com_arr[my_members, 0], com_arr[my_members, 2], color=col, s=0.5
        )
        _ = axarr[2].plot(
            cluster_center[0],
            cluster_center[1],
...
def graph_cluster_and_tree_info_vs_time(axarr, time_arr, trees):
    ...
    for tree in trees:
        ...
        _ = axarr[1, 0].scatter(flat_time_lst, clust_ids, s=0.01, marker=".")
