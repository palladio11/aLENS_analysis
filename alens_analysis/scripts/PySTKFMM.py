...
pvfmmlib = cdll.LoadLibrary(Path.home() / 'local/lib64/libpvfmm.so')
lib = cdll.LoadLibrary(Path.home() / 'local/lib64/libSTKFMM_SHARED.so')
...
class Stk3DFMM():
    def __init__(self, mult_order, max_pts, pbc, kernels):
        ...
        self.fmm = c_void_p(lib.Stk3DFMM_create(
            self.mult_order, self.max_pts, self.pbc, self.kernels))

    def __del__(self):
        lib.Stk3DFMM_destroy(self.fmm)

    def set_box(self, origin, length):
        lib.Stk3DFMM_set_box(self.fmm, origin.ctypes.data_as(
            POINTER(c_double)), c_double(length))

    def get_kernel_dimension(self, kernel):
        dims = np.zeros(3, dtype='int32')
        lib.Stk3DFMM_get_kernel_dimension(
            c_int(kernel), dims.ctypes.data_as(POINTER(c_int)))
        return dims
...
class StkWallFMM():
    def __init__(self, mult_order, max_pts, pbc, kernels):
        ...
        self.fmm = c_void_p(lib.StkWallFMM_create(
            self.mult_order, self.max_pts, self.pbc, self.kernels))
    ...
    def set_box(self, origin, length):
        lib.StkWallFMM_set_box(self.fmm, origin.ctypes.data_as(
            POINTER(c_double)), c_double(length))

    def get_kernel_dimension(self, kernel):
        dims = np.zeros(3, dtype='int32')
        lib.StkWallFMM_get_kernel_dimension(
            c_int(kernel), dims.ctypes.data_as(POINTER(c_int)))
        return dims
...
class DArray():
    def __init__(self, array):
        ...
        self.chunk = np.empty(
            (self.split_sizes[self.rank] // self.dim, self.dim), dtype='float64')

    def scatter(self):
        self.comm.Scatterv([self.data, self.split_sizes,
                            self.displacements, MPI.DOUBLE], self.chunk, root=0)

    def gather(self):
        self.comm.Gatherv(self.chunk, [
                          self.data, self.split_sizes, self.displacements, MPI.DOUBLE], root=0)
    ...
    def update_split_sizes(self):
        ...
        for i in range(1, self.size):
            self.displacements[i] = self.displacements[i -
                                                       1] + self.split_sizes[i-1]
